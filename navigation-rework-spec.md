...
Status: design in progress, nothing here is built yet unless stated otherwise. This sits alongside `spec.md` rather than replacing any of it, and covers the generalisation of the Journal `ViewTrack`/Navigator proof of concept into the app's whole navigation model.

## Terminology

- `ViewTrackOperator`: the single top level component wrapping the whole main part of the app (Journal and Collections both). Owns `currentTab`, coordinates all motion between `ViewTrack`s and pages, and is what disables pagination while a page is being edited.
- `ViewTrack`: a horizontal scrolling area filled with `ViewPage`s, this is what the Journal pager already is in the proof of concept (so `JournalPager` becomes `JournalViewTrack`, and so on for `CollectionsViewTrack`, `ItemsViewTrack`, the add flow's own `AddPageViewTrack`).
- Navigator (exact naming still settling): the small strip along the bottom of a `ViewTrack` used to jump between its pages, kept strictly to that one job now that edit mode no longer overloads it. This is what the existing day circle strip (`JournalTrack`) becomes.
- `ViewPage`: a single page within a `ViewTrack`.

A given `ViewTrack` is always wired to one specific Navigator by hand (the Collections `ViewTrack` always talks to the Collections Navigator), there is no generic pairing mechanism.

## Where this starts from

The Journal `ViewTrack` and Navigator proof of concept already works and is the foundation for everything below:

- A shared `progress` value (a continuous float index) and an `activeDriver` flag (which side, the `ViewTrack` or the Navigator, is currently being dragged) keep the two in sync, whichever side is dragged drives the other.
...
Generalises "a `ViewTrack` plus its Navigator" into any number of stacked, absolutely positioned `ViewTrack`s, with only one ever live and interactive outside of a transition:
...
The trigger changes though. Rather than tapping the already active item on the Navigator (today's overload, and part of what's making the Navigator's gestures fragile), each editable `ViewPage` gets its own small "Edit" button pinned to the bottom of the page, inside an `AnimateHeight` container so it collapses away the moment edit mode starts and reappears once it ends. Detecting edit mode from an overscroll past the bottom of the page (mirroring pull to refresh at the top) is a nice future direction but deliberately deferred, for now the button is the entire mechanism, so short or empty pages need no special handling.
...
1. Rename and reorganise the existing proof of concept into the terminology above (`JournalPager` becomes `JournalViewTrack`, `JournalTrack` becomes the Journal Navigator, both moved into `/views/`), a pure refactor with no behaviour change, so everything after this references the new names.
2. Add the Motion Style setting (Immersive / Instant) end to end on the existing single Journal `ViewTrack`, this doesn't depend on anything else in this list.
3. Replace edit mode's trigger with the static Edit button on each page, keeping its current visual behaviour (Navigator shifting up to reveal cancel and save) exactly as is.
...
5. Wire the add-day flow through that mechanism (the add button triggers a transition down, save uses the immediate mode jump into the `ViewTrack` above, cancel reverses it).
